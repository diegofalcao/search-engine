...
//                        entryTmp->magnitude += (term->idf * documentTmp->tf) * (term->idf * documentTmp->tf);
                        entryTmp->magnitude += (term->idf * (1 + log(documentTmp->tf))) * (term->idf * (1 + log(documentTmp->tf)));
 ...
 */
int getTermFrequencyOnQuery(char *query, char *term) {
	char *p = query;
...
void searchByVectorModel(char termName[]) {
    ...
    while (token != NULL) {
        ...
        int termFrequencyOnQuery = getTermFrequencyOnQuery(cpTermName, token);
...
//				entry->sum = (term->idf * document->tf) * (term->idf * termFrequencyOnQuery);
                entry->sum = (term->idf * (1 + log(document->tf))) * (term->idf * termFrequencyOnQuery);
