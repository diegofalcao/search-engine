...
void sortEntriesByCosDesc(Entry *values[]) {
    ...
    for (i = 0; i < NUM_OF_DOCUMENTS; ++i) {
        for (j = i + 1; j < NUM_OF_DOCUMENTS; ++j) {
            ...
            double cos1 = values[i]->sum / values[i]->magnitude;
            double cos2 = values[j]->sum / values[j]->magnitude;
 ...
 *
 * This evaluation executes 50 queries from a txt file and evaluates the results
 * comparing them with a file containing the relevant results for each of these queries.
 */
void evaluateModelByMAPAndPat10() {
    ...
    for(i = 0; i < NUMBER_OF_QUERIES_TO_EVAL; i++) { // Fix to the normal value: 50
        ...
        int newSize = strlen(path)  + strlen(strQueryNumber) + strlen(".txt") + 1;
        ...
        if (file != NULL) {
            ...
            while (fgets(line, sizeof line, file) != NULL ) {
                ...
                resultsToEvaluate = searchByVectorModel(line, false, resultsToEvaluate); // each line of the file is a query
...
int main(void) {
    ...
    int result = processXMLData("../dataset/textDescDafitiPosthaus.xml");
    ...
    while (true) {
        printf("\nPlease, input the text to search," ANSI_COLOR_YELLOW " !m " ANSI_COLOR_RESET "for model mestrics and " ANSI_COLOR_RED "!q" ANSI_COLOR_RESET " to exit: ");
